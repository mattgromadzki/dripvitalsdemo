...
export default function LoginPage() {
  ...
  async function submit() {
    setBusy(true);
    setErr(null);
    const res = await login(email, password, twofa ? code : undefined);
    if (res.ok) { router.replace("/dashboard"); return; }
    if (res.twofa) { setTwofa(true); if (res.error) setErr(res.error); setBusy(false); return; }
    setErr(res.error || "Sign in failed."); setBusy(false);
  }
  ...
  return (
    <AuthShell title="Sign in" subtitle="Provider & staff console">
      ...
      {twofa ? (
        <>
          <div className="mb-3 text-[12.5px] text-ink-muted leading-relaxed">Enter the 6-digit code from your authenticator app for <b className="text-ink">{email}</b>. You can also use a backup code.</div>
          <label className="fl">Authentication code</label>
          <input className="fi mb-4" inputMode="numeric" autoFocus value={code} autoComplete="one-time-code" onChange={(e) => setCode(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()} placeholder="123 456" />
          <button className="btn btn-primary w-full" onClick={submit} disabled={busy}>{busy ? "Verifying…" : "Verify & sign in"}</button>
          <button onClick={() => { setTwofa(false); setCode(""); setErr(null); }} className="block w-full text-center text-[12px] text-brand font-medium mt-3">← Use a different account</button>
        </>
      ) : (
        <>
